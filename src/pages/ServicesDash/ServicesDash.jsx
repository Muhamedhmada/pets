...
export default function ServicesDash() {
  ...
  const createBenefit = async () => {
    ...
    try {
      ...
      if (imgRes.data.status === "success") {
        ...
        const res = await axios.post(base_url + "create_service", benefitData, {
          headers: {
            lang: "es",
          },
        });
  ...
  const updateBenefit = async () => {
    ...
    try {
      ...
      await axios.post(base_url + `update_service/${currentId}`, benefitData, {
        headers: {
          lang: "es",
        },
      });
  ...
  const updateStatus = async (id) => {
    setLoading(true);
    try {
      const res = await axios.get(`${base_url}update_service_status/${id}`);
      if (res.status === 200) {
  ...
  return (
    <>
      <Modal
        title={editMode ? "Editar servicio" : "Agregar servicio"}
        show={isOpenModal}
